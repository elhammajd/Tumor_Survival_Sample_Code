...
# =========================================================================
Datasets= "./Datasets"  
filename0=paste0(Datasets,"/32/patinfo")
wd<- paste0(Datasets,"/32/patinfo.sas7bdat")
dm<- assign(filename0, read_sas(wd))


base <- unique(dm[,c("patid" , "SEX", "AGE_C")])
names(base)[names(base) == "AGE_C"] <- "AGE"
names(base)[names(base) == "patid"] <- "USUBJID"
base[base == ""] <- NA
...
write.csv(base,"./cvresults/32-base.csv")

# ==========================================================================
# Actual Tumor Size
# ==========================================================================
filename0=paste0(Datasets,"/32/turesp")
wd<- paste0(Datasets,"/32/turesp.sas7bdat")
dtr<- assign(filename0, read_sas(wd))



TumorSz <- data.frame( USUBJID=dtr$patid, val=dtr$TULDSUM , visitN = dtr$VISIT)
...
write.csv(PTumorch, "./cvresults/32-FullTumorSz.csv")


keeps <- c("USUBJID", "val.2.01", "val.3.01", "val.4.01")

TumorSzfinal =PTumorch[keeps]
names(TumorSzfinal)[names(TumorSzfinal)=="val.2.01"] <-"v2"
names(TumorSzfinal)[names(TumorSzfinal)=="val.3.01"]<- "v3"
names(TumorSzfinal)[names(TumorSzfinal)=="val.4.01"]<- "v4"

write.csv(TumorSzfinal, "./cvresults/32-TumorSz.csv")
# ========================================================================
# PCA MODEL
# ========================================================================
TumorSzV3<-read.csv("./cvresults/32-TumorSz.csv")
keeps <-c("v2","v3")
...
a<- matrix(a)
colnames(a)<-paste0("model32")
write.csv(a,"./val_v3/32.csv", append = FALSE, quote = FALSE, sep = "", row.names = FALSE, col.names = TRUE)
scorevisit3<- cbind(USUBJID=TumorSzfinal$USUBJID, scorevisit3)


write.csv(scorevisit3,"./cvresults/32-pca3.csv")
# ========================================================================
keeps <- c("v2", "v3", "v4")
...
a<- matrix(a)
colnames(a)<-paste0("model32")
write.csv(a,"./val_v4/32.csv", append = FALSE, quote = FALSE, sep = "", row.names = FALSE, col.names = TRUE)
scorevisit4<- cbind(USUBJID=TumorSzfinal$USUBJID, scorevisit4)


write.csv(scorevisit4,"./cvresults/32-pca4.csv")
# =========================================================================
# Overall survival
# =========================================================================
...
write.csv(os, "./cvresults/32-os.csv")

# =================================================================
# best overall response T=CR, T=PR, F=SD, F=PD, NE -> NA
# ================================================================= 
filename0=paste0(Datasets,"/32/radiothy")
wd<- paste0(Datasets,"/32/radiothy.sas7bdat")
adrs<- assign(filename0, read_sas(wd))


BestResp <- unique(data.frame(USUBJID=adrs$patid, BORval=adrs$RTRESP))
...
BestResp["BORval"][BestResp["BORval"]==5]<-0
BestResp["BORval"][BestResp["BORval"]==6]<-NA
BestResp["BORval"][BestResp["BORval"]==99]<-NA
BestResp<- na.omit(BestResp)
...
write.csv(BestResp, "./cvresults/32-BestResp.csv")

# ==================================================================
# Merge all data  visits 
# ==================================================================
base<-read.csv("./cvresults/32-base.csv")
os<-read.csv("./cvresults/32-os.csv")
BestResp<-read.csv("./cvresults/32-BestResp.csv")
pca3<-read.csv("./cvresults/32-pca3.csv")
pca4<-read.csv("./cvresults/32-pca4.csv")
...
drop <- c("X.x","X.y")
write.csv(alldata, "./cvresults/32-alldata0.csv")
alldata<- merge(TumorSzfinal, alldata, by="USUBJID")
write.csv(alldata, "./cvresults/32-alldata32.csv")
alldata <- merge(os, alldata, by="USUBJID")
alldata = alldata[,!(names(alldata) %in% drop)]


write.csv(alldata, "./cvresults/32-alldata.csv")

# ============================================================================
# Prediction / Cross validation for PCA Model 3 Visits
# ============================================================================
alldata<-read.csv("./cvresults/32-alldata.csv")
...
glmnet.bin <- function(data,  idx.tr=NULL, base.covariates= baseCov, lambda=c("lambda.min", "lambda.1se")){
  ...
  # tidy(bestcut)
  # write.csv( tidy(bestcut),  "./cvresults/32-bestcut_V3.csv")
  coef.exactt <- coef(cv.fit, s = c(lambda))
  
  # tidy(coef.exactt)
  # write.csv( tidy(coef.exactt),  "./cvresults/32-coef_V3.csv")
...
write.csv(conf_matrix, "./cvresults/32-PCA2_conf_matrix.csv")
...
write.table( bestcut,  "./cvresults/32bestcut_V3.csv")
write.table(coef_intercept, "./cvresults/32Intercept_V3.csv")
write.table(coef_sex, "./cvresults/32SEX_V3.csv")
write.table(coef_age, "./cvresults/32AGE_V3.csv")
write.table(coef_pc1, "./cvresults/32PC1_V3.csv")
write.csv(PCAV2Prediction, "./cvresults/32PCAV2Prediction_V3.csv")
write.table(misclassificationrate, "./cvresults/32misclassificationrate_V3.csv")
write.table(auc, "./cvresults/32auc_V3.csv")
write.table(sensitivity, "./cvresults/32sensitivity_V3.csv")
write.table(specificity, "./cvresults/32specificity_V3.csv")
# ============================================================================
...
glmnet.bin <- function(data, idx.tr=NULL, base.covariates= baseCov, lambda=c("lambda.min", "lambda.1se")){
  ...
  # tidy(bestcut)
  # write.csv( tidy(bestcut),  "./cvresults/32-bestcut_V4.csv")
  coef.exactt <- coef(cv.fit, s = c(lambda))
  
  # tidy(coef.exactt)
  # write.csv( tidy(coef.exactt),  "./cvresults/32-coef_V4.csv")
...
specificity<- specificity(conf_matrix)
conf_matrix<- as.numeric(conf_matrix)
write.csv(conf_matrix, "./cvresults/32-PCA3_conf_matrix.csv")
...
write.table( bestcut,  "./cvresults/32bestcut_V4.csv")
write.table(coef_intercept, "./cvresults/32Intercept_V4.csv")
write.table(coef_sex, "./cvresults/32SEX_V4.csv")
write.table(coef_age, "./cvresults/32AGE_V4.csv")
write.table(coef_pc1, "./cvresults/32PC1_V4.csv")
write.table(coef_pc2, "./cvresults/32PC2_V4.csv")
write.csv(PCAV3Prediction, "./cvresults/32PCAV3Prediction_V4.csv")
write.table(misclassificationrate, "./cvresults/32misclassificationrate_V4.csv")
write.table(auc, "./cvresults/32auc_V4.csv")
write.table(sensitivity, "./cvresults/32sensitivity_V4.csv")
write.table(specificity, "./cvresults/32specificity_V4.csv")
# ======================================================================================