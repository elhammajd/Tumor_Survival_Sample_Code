...
# =========================================================================
Datasets= "./Datasets"  
filename0=paste0(Datasets,"/17/adsl")
wd<- paste0(Datasets,"/17/adsl.sas7bdat")
dm<- assign(filename0, read_sas(wd))


base <- unique(dm[,c("USUBJID" , "SEXN", "AGEGR1N")])
names(base)[names(base) == "SEXN"] <- "SEX"
names(base)[names(base) == "AGEGR1N"] <- "AGE"
base[base == ""] <- NA
...
write.csv(base,"./cvresults/17-base.csv")

# ==========================================================================
# Actual Tumor Size
# ==========================================================================
filename0=paste0(Datasets,"/17/adtr")
wd<- paste0(Datasets,"/17/adtr.sas7bdat")
dtr<- assign(filename0, read_sas(wd))


TumorSz <- data.frame( USUBJID=dtr$USUBJID, val=dtr$PCHG , visitN = round(dtr$AVISITN))
...
write.csv(TumorSz, "./cvresults/17-FullTumorSz.csv")

keeps <- c("USUBJID","val.275", "val.475", "val.675", "val.875")

TumorSzfinal =TumorSz[keeps]
names(TumorSzfinal)[names(TumorSzfinal)=="val.275"] <-"v2"
names(TumorSzfinal)[names(TumorSzfinal)=="val.475"]<- "v3"
names(TumorSzfinal)[names(TumorSzfinal)=="val.675"]<- "v4"

write.csv(TumorSzfinal, "./cvresults/17-TumorSz.csv")
# ========================================================================
# PCA MODEL
# ========================================================================
TumorSzV3<-read.csv("./cvresults/17-TumorSz.csv")
keeps <-c("v2","v3")
...
a<- matrix(a)
colnames(a)<-paste0("model17")
write.csv(a,"./val_v3/17.csv", append = FALSE, quote = FALSE, sep = "", row.names = FALSE, col.names = TRUE)
scorevisit3<- cbind(USUBJID=TumorSzfinal$USUBJID, scorevisit3)

write.csv(scorevisit3,"./cvresults/17-pca3.csv")
# ========================================================================
keeps <- c("v2", "v3", "v4")
...
a<- matrix(a)
colnames(a)<-paste0("model17")
write.csv(a,"./val_v4/17.csv", append = FALSE, quote = FALSE, sep = "", row.names = FALSE, col.names = TRUE)
scorevisit4<- cbind(USUBJID=TumorSzfinal$USUBJID, scorevisit4)


write.csv(scorevisit4,"./cvresults/17-pca4.csv")
...
write.csv(os, "./cvresults/17-os.csv")

# =================================================================
# best overall response T=CR, T=PR, F=SD, F=PD, NE -> NA
# ================================================================= 
filename0=paste0(Datasets,"/17/adrs")
wd<- paste0(Datasets,"/17/adrs.sas7bdat")
adrs<- assign(filename0, read_sas(wd))


idx<- adrs$PARAM =="Best Overall Response ORR"

BestResp <- unique(data.frame(USUBJID=adrs$USUBJID[idx], BORval=adrs$AVALC[idx]))
...
BestResp["BORval"][BestResp["BORval"]=="NE"]<-NA
BestResp["BORval"][BestResp["BORval"]=="SD"]<-"0"
BestResp["BORval"][BestResp["BORval"]=="PD"]<-"0"
BestResp["BORval"][BestResp["BORval"]=="NonCR/NonPD"]<-"0"
BestResp["BORval"][BestResp["BORval"]=="Unconfirmed PR"]<-NA
BestResp["BORval"][BestResp["BORval"]=="41"]<-NA
BestResp<- na.omit(BestResp)
...
write.csv(BestResp, "./cvresults/17-BestResp.csv")

# ==================================================================
# Merge all data  visits 
# ==================================================================
TumorSz<-read.csv("./cvresults/17-TumorSz.csv")
base<-read.csv("./cvresults/17-base.csv")
os<-read.csv("./cvresults/17-os.csv")
BestResp<-read.csv("./cvresults/17-BestResp.csv")
pca3<-read.csv("./cvresults/17-pca3.csv")
pca4<-read.csv("./cvresults/17-pca4.csv")
...
drop <- c("X.x","X.y")
write.csv(alldata, "./cvresults/17-alldata0.csv")
alldata<- merge(TumorSzfinal, alldata, by="USUBJID")
write.csv(alldata, "./cvresults/17-alldata17.csv")
alldata <- merge(os, alldata, by="USUBJID")
alldata = alldata[,!(names(alldata) %in% drop)]


write.csv(alldata, "./cvresults/17-alldata.csv")

# ============================================================================
# Prediction / Cross validation for PCA Model 3 Visits
# ============================================================================
alldata<-read.csv("./cvresults/17-alldata.csv")
...
glmnet.bin <- function(data,  idx.tr=NULL, base.covariates= baseCov, lambda=c("lambda.min", "lambda.1se")){
  ...
  # tidy(bestcut)
  # write.csv( tidy(bestcut),  "./cvresults/17-bestcut_V3.csv")
  coef.exactt <- coef(cv.fit, s = c(lambda))
  
  # tidy(coef.exactt)
  # write.csv( tidy(coef.exactt),  "./cvresults/17-coef_V3.csv")
...
write.csv(conf_matrix, "./cvresults/17-PCA2_conf_matrix.csv")
...
write.table( bestcut,  "./cvresults/17bestcut_V3.csv")
write.table(coef_intercept, "./cvresults/17Intercept_V3.csv")
write.table(coef_sex, "./cvresults/17SEX_V3.csv")
write.table(coef_age, "./cvresults/17AGE_V3.csv")
write.table(coef_pc1, "./cvresults/17PC1_V3.csv")
write.csv(PCAV2Prediction, "./cvresults/17PCAV2Prediction_V3.csv")
write.table(misclassificationrate, "./cvresults/17misclassificationrate_V3.csv")
write.table(auc, "./cvresults/17auc_V3.csv")
write.table(sensitivity, "./cvresults/17sensitivity_V3.csv")
write.table(specificity, "./cvresults/17specificity_V3.csv")
# ============================================================================
...
glmnet.bin <- function(data, idx.tr=NULL, base.covariates= baseCov, lambda=c("lambda.min", "lambda.1se")){
  ...
  # tidy(bestcut)
  # write.csv( tidy(bestcut),  "./cvresults/17-bestcut_V4.csv")
  coef.exactt <- coef(cv.fit, s = c(lambda))
  
  # tidy(coef.exactt)
  # write.csv( tidy(coef.exactt),  "./cvresults/17-coef_V4.csv")
...
specificity<- specificity(conf_matrix)
conf_matrix<- as.numeric(conf_matrix)
write.csv(conf_matrix, "./cvresults/17-PCA3_conf_matrix.csv")
...
write.table( bestcut,  "./cvresults/17bestcut_V4.csv")
write.table(coef_intercept, "./cvresults/17Intercept_V4.csv")
write.table(coef_sex, "./cvresults/17SEX_V4.csv")
write.table(coef_age, "./cvresults/17AGE_V4.csv")
write.table(coef_pc1, "./cvresults/17PC1_V4.csv")
write.table(coef_pc2, "./cvresults/17PC2_V4.csv")
write.csv(PCAV3Prediction, "./cvresults/17PCAV3Prediction_V4.csv")
write.table(misclassificationrate, "./cvresults/17misclassificationrate_V4.csv")
write.table(auc, "./cvresults/17auc_V4.csv")
write.table(sensitivity, "./cvresults/17sensitivity_V4.csv")
write.table(specificity, "./cvresults/17specificity_V4.csv")
# ==================================================================================
