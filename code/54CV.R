...
# =========================================================================
Datasets= "./Datasets"  
filename0=paste0(Datasets,"/54/dm")
wd<- paste0(Datasets,"/54/dm.sas7bdat")
dm<- assign(filename0, read_sas(wd))


base <- unique(dm[,c("PT" , "SEX", "AGE")])
names(base)[names(base) == "PT"] <- "USUBJID"
...
write.csv(base,"./cvresults/54-base.csv")

# ==========================================================================
# Actual Tumor Size
# ==========================================================================
filename0=paste0(Datasets,"/54/lesles")
wd<- paste0(Datasets,"/54/lesles.sas7bdat")
dtr<- assign(filename0, read_sas(wd))

TumorSz <- data.frame( USUBJID=dtr$PT, val=dtr$DIASUM , visitN = dtr$VISITNUM)
...
write.csv(PTumorch, "./cvresults/54-FullTumorSz.csv")


keeps <- c("USUBJID","val.200","val.201","val.202")

TumorSzfinal =PTumorch[keeps]
names(TumorSzfinal)[names(TumorSzfinal)=="val.200"] <-"v2"
names(TumorSzfinal)[names(TumorSzfinal)=="val.201"]<- "v3"
names(TumorSzfinal)[names(TumorSzfinal)=="val.202"]<- "v4"

write.csv(TumorSzfinal, "./cvresults/54-TumorSz.csv")
# ========================================================================
# PCA MODEL
# ========================================================================
TumorSzV3<-read.csv("./cvresults/54-TumorSz.csv")
keeps <-c("v2","v3")
...
a<- matrix(a)
colnames(a)<-paste0("model54")
write.csv(a,"./val_v3/54.csv", append = FALSE, quote = FALSE, sep = "", row.names = FALSE, col.names = TRUE)
scorevisit3<- cbind(USUBJID=TumorSzfinal$USUBJID, scorevisit3)


write.csv(scorevisit3,"./cvresults/54-pca3.csv")
# ========================================================================
keeps <- c("v2", "v3", "v4")
...
a<- matrix(a)
colnames(a)<-paste0("model54")
write.csv(a,"./val_v4/54.csv", append = FALSE, quote = FALSE, sep = "", row.names = FALSE, col.names = TRUE)
scorevisit4<- cbind(USUBJID=TumorSzfinal$USUBJID, scorevisit4)


write.csv(scorevisit4,"./cvresults/54-pca4.csv")
# =========================================================================
# Overall survival
# =========================================================================
...
write.csv(os, "./cvresults/54-os.csv")

# =================================================================
# best overall response T=CR, T=PR, F=SD, F=PD, NE -> NA
# ================================================================= 
filename0=paste0(Datasets,"/54/rsp")
wd<- paste0(Datasets,"/54/rsp.sas7bdat")
adrs<- assign(filename0, read_sas(wd))


BestResp <- (data.frame(USUBJID=adrs$PT, BORval=adrs$ORSP))
...
BestResp["BORval"][BestResp["BORval"]=="UNK"]<-NA
BestResp["BORval"][BestResp["BORval"]=="ND"]<-NA
...
write.csv(BestRespf, "./cvresults/54-BestResp.csv")

# ==================================================================
# Merge all data  visits 
# ==================================================================
base<-read.csv("./cvresults/54-base.csv")
os<-read.csv("./cvresults/54-os.csv")
BestResp<-read.csv("./cvresults/54-BestResp.csv")
pca3<-read.csv("./cvresults/54-pca3.csv")
pca4<-read.csv("./cvresults/54-pca4.csv")
...
drop <- c("X.x","X.y")
write.csv(alldata, "./cvresults/54-alldata0.csv")
alldata<- merge(TumorSzfinal, alldata, by="USUBJID")
write.csv(alldata, "./cvresults/54-alldata54.csv")
alldata <- merge(os, alldata, by="USUBJID")
alldata = alldata[,!(names(alldata) %in% drop)]
alldata <- merge(os, alldata, by="USUBJID")


write.csv(alldata, "./cvresults/54-alldata.csv")

# ============================================================================
# Prediction / Cross validation for PCA Model 3 Visits
# ============================================================================
alldata<-read.csv("./cvresults/54-alldata.csv")
...
glmnet.bin <- function(data,  idx.tr=NULL, base.covariates= baseCov, lambda=c("lambda.min", "lambda.1se")){
  ...
  idx0 <- sample(1:20, nrow(xx.tr),replace=T)
  for (jj in 1:10) {
  ...
  # tidy(bestcut)
  # write.csv( tidy(bestcut),  "./cvresults/54-bestcut_V3.csv")
  coef.exactt <- coef(cv.fit, s = c(lambda))
  
  # tidy(coef.exactt)
  # write.csv( tidy(coef.exactt),  "./cvresults/54-coef_V3.csv")
...
write.csv(conf_matrix, "./cvresults/54-PCA2_conf_matrix.csv")
...
write.table( bestcut,  "./cvresults/54bestcut_V3.csv")
write.table(coef_intercept, "./cvresults/54Intercept_V3.csv")
write.table(coef_sex, "./cvresults/54SEX_V3.csv")
write.table(coef_age, "./cvresults/54AGE_V3.csv")
write.table(coef_pc1, "./cvresults/54PC1_V3.csv")
write.csv(PCAV2Prediction, "./cvresults/54PCAV2Prediction_V3.csv")
write.table(misclassificationrate, "./cvresults/54misclassificationrate_V3.csv")
write.table(auc, "./cvresults/54auc_V3.csv")
write.table(sensitivity, "./cvresults/54sensitivity_V3.csv")
write.table(specificity, "./cvresults/54specificity_V3.csv")
# ============================================================================
...
glmnet.bin <- function(data, idx.tr=NULL, base.covariates= baseCov, lambda=c("lambda.min", "lambda.1se")){
  ...
  # tidy(bestcut)
  # write.csv( tidy(bestcut),  "./cvresults/54-bestcut_V4.csv")
  coef.exactt <- coef(cv.fit, s = c(lambda))
  
  # tidy(coef.exactt)
  # write.csv( tidy(coef.exactt),  "./cvresults/54-coef_V4.csv")
...
specificity<- specificity(conf_matrix)
conf_matrix<- as.numeric(conf_matrix)
write.csv(conf_matrix, "./cvresults/54-PCA3_conf_matrix.csv")
...
write.table( bestcut,  "./cvresults/54bestcut_V4.csv")
write.table(coef_intercept, "./cvresults/54Intercept_V4.csv")
write.table(coef_sex, "./cvresults/54SEX_V4.csv")
write.table(coef_age, "./cvresults/54AGE_V4.csv")
write.table(coef_pc1, "./cvresults/54PC1_V4.csv")
write.table(coef_pc2, "./cvresults/54PC2_V4.csv")
write.csv(PCAV3Prediction, "./cvresults/54PCAV3Prediction_V4.csv")
write.table(misclassificationrate, "./cvresults/54misclassificationrate_V4.csv")
write.table(auc, "./cvresults/54auc_V4.csv")
write.table(sensitivity, "./cvresults/54sensitivity_V4.csv")
write.table(specificity, "./cvresults/54specificity_V4.csv")
# ====================================================================================