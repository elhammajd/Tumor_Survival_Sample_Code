...
# =========================================================================
Datasets= "./Datasets"  
filename0=paste0(Datasets,"/58/dm")
wd<- paste0(Datasets,"/58/dm.sas7bdat")
...
write.csv(base,"./cvresults/58-base.csv")

# ==========================================================================
# Actual Tumor Size
# ==========================================================================
filename0=paste0(Datasets,"/58/tr")
wd<- paste0(Datasets,"/58/tr.sas7bdat")
dtr<- assign(filename0, read_sas(wd))


idx<- dtr$TRTESTCD== "SUMLDIAM"
TumorSz <- data.frame( USUBJID=dtr$USUBJID[idx], val=dtr$TRSTRESN[idx], visitN = dtr$VISITNUM[idx])
...
print(PTumorch)
write.csv(PTumorch, "./cvresults/58-FullTumorSz.csv")


keeps <- c("USUBJID","val.156","val.328","val.528")
print(keeps)
TumorSzfinal =PTumorch[keeps]
names(TumorSzfinal)[names(TumorSzfinal)=="val.156"] <-"v2"
names(TumorSzfinal)[names(TumorSzfinal)=="val.328"]<- "v3"
names(TumorSzfinal)[names(TumorSzfinal)=="val.528"]<- "v4"
TumorSzfinal$USUBJID<- as.numeric(TumorSzfinal$USUBJID)
write.csv(TumorSzfinal, "./cvresults/58-TumorSz.csv")
# ========================================================================
# PCA MODEL
# ========================================================================
TumorSzV3<-read.csv("./cvresults/58-TumorSz.csv")
keeps <-c("v2","v3")
...
a<-pca3$loadings
a<- matrix(a)
colnames(a)<-paste0("model58")
#write.csv(a,"./Resultmethod1/val3-58.csv", append = FALSE, quote = FALSE, sep = "", row.names = FALSE, col.names = TRUE)
scorevisit3<- cbind(USUBJID=TumorSzfinal$USUBJID, scorevisit3)

write.csv(scorevisit3,"./cvresults/58-pca3.csv")
# ========================================================================
keeps <- c("v2", "v3", "v4")
...
a<- matrix(a)
colnames(a)<-paste0("model58")
#write.csv(a,"./Resultmethod1/val4-58.csv", append = FALSE, quote = FALSE, sep = "", row.names = FALSE, col.names = TRUE)
scorevisit4<- cbind(USUBJID=TumorSzfinal$USUBJID, scorevisit4)

write.csv(scorevisit4,"./cvresults/58-pca4.csv")
# =========================================================================
# Overall survival
# =========================================================================
...
write.csv(os, "./cvresults/58-os.csv")
# =================================================================
# best overall response T=CR, T=PR, F=SD, F=PD, NE -> NA
# ================================================================= 
filename0=paste0(Datasets,"/58/rs")
wd<- paste0(Datasets,"/58/rs.sas7bdat")
adrs<- assign(filename0, read_sas(wd))

idx<- adrs$RSTESTCD=="OVRLRESP" & adrs$RSGRPID=="FINAL A~"
BestResp <- (data.frame(USUBJID=adrs$USUBJID, BORval=adrs$RSSTRESC))
...
BestResp["BORval"][BestResp["BORval"]=="NON-CR/SD"]<-"0"
BestResp["BORval"][BestResp["BORval"]=="NOT ASSESSABLE"]<-NA
BestResp["BORval"][BestResp["BORval"]=="NOT APPLICABLE"]<-NA
BestResp["BORval"][BestResp["BORval"]=="N"]<-NA
BestResp["BORval"][BestResp["BORval"]=="Y"]<-NA
...
write.csv(BestRespf, "./cvresults/58-BestResp.csv")
# ==================================================================
# Merge all data  visits 
# ==================================================================
base<-read.csv("./cvresults/58-base.csv")
os<-read.csv("./cvresults/58-os.csv")
BestResp<-read.csv("./cvresults/58-BestResp.csv")
pca3<-read.csv("./cvresults/58-pca3.csv")
pca4<-read.csv("./cvresults/58-pca4.csv")
...
drop <- c("X.x","X.y")
alldata$USUBJID<- as.numeric(alldata$USUBJID)
write.csv(alldata, "./cvresults/58-alldata0.csv")
alldata <- merge(TumorSzfinal, alldata, by="USUBJID")
write.csv(alldata, "./cvresults/58-alldata58.csv")
...
# ============================================================================
alldata<-read.csv("./cvresults/58-alldata58.csv")
alldata<-as.data.frame(alldata)

adds<- list(c())
baseCov= c("SEX", "AGE", "pc1")
glmnet.bin <- function(data, add.covariates, idx.tr=NULL, base.covariates= baseCov, lambda=c("lambda.min", "lambda.1se")){
  lambda <- match.arg(lambda)
  if (is.null(idx.tr)) idx.tr <- 1:nrow(data)
  yy <- as.matrix(data[,c("BORval")])
  preds <- c( base.covariates, add.covariates)
  xx <- (data[,preds])
  ...
  return(list(predicted=pp, bestcut=bestcut, cvfit=cv.fit, predictor=xx))
...
colnames(res) <- paste0("model1",1:nmodel)
set.seed(0)
BB <- 10 #ceiling(NN/10)
...
res1 <- res
for(ii in 1:BB) {
  for( jj in 1:length(adds)) res1[which(index==ii),jj] <- glmnet.bin(alldata, add.covariates= NULL, idx.tr=(which(index!=ii)))$predicted
}
...
#write.csv(PCAV3Prediction, "./cvresults/58PCAV3Prediction_V3.csv")
#write.table(misclassificationrate, "./cvresults/misclassificationrate_V3.csv")
#write.table(auc, "./cvresults/auc_V3.csv")
#write.table(sensitivity, "./cvresults/sensitivity_V3.csv")
#write.table(specificity, "./cvresults/specificity_V3.csv")
...
nmodel <- length(adds)
res <- matrix(NA, NN, nmodel)
colnames(res) <- paste0("model3",1:nmodel)
set.seed(0)
BB <- 10 #ceiling(NN/10)
index <- sample(1:BB, NN, replace=T)
...
res3 <- res
for(ii in 1:BB) {
  for( jj in 1:length(adds)) res3[which(index==ii),jj] <- glmnet.bin(alldata, add.covariates= NULL, idx.tr=(which(index!=ii)))$predicted
}
#misclassification rate
...
#write.csv(PCAV3Prediction, "./cvresults/58PCAV3Prediction_V4.csv")
#write.table(misclassificationrate, "./cvresults/misclassificationrate_V4.csv")
#write.table(auc, "./cvresults/auc_V4.csv")
#write.table(sensitivity, "./cvresults/sensitivity_V4.csv")
#write.table(specificity, "./cvresults/specificity_V4.csv")
# ===============================================================================
 