...
# =========================================================================
Datasets= "./Datasets"  
filename0=paste0(Datasets,"/34/addm")
wd<- paste0(Datasets,"/34/addm.sas7bdat")
dm<- assign(filename0, read_sas(wd))


base <- unique(dm[,c("USUBJID" , "SEXN", "AGEGRP")])
names(base)[names(base) == "SEXN"] <- "SEX"
names(base)[names(base) == "AGEGRP"] <- "AGE"
base["AGE"][base["AGE"]==">=85"]<- "86"
base[base == ""] <- NA
...
write.csv(base,"./cvresults/34-base.csv")

# ==========================================================================
# Actual Tumor Size
# ==========================================================================
filename0=paste0(Datasets,"/34/adls")
wd<- paste0(Datasets,"/34/adls.sas7bdat")
dtr<- assign(filename0, read_sas(wd))


idx<- dtr$LSTESTCD=="LENGTH"
TumorSz <- data.frame( USUBJID=dtr$USUBJID[idx], val=dtr$LSSTRESN[idx], visitN = dtr$VISITNUM[idx])
...
TumorCh<- ((TumorSz[2:ncol(TumorSz)]-TumorSz[,3])/ TumorSz[,3])*100  
...
write.csv(PTumorch, "./cvresults/34-FullTumorSz.csv")


keeps <- c("USUBJID","val.4","val.5","val.16")

TumorSzfinal =PTumorch[keeps]
names(TumorSzfinal)[names(TumorSzfinal)=="val.4"] <-"v2"
names(TumorSzfinal)[names(TumorSzfinal)=="val.5"]<- "v3"
names(TumorSzfinal)[names(TumorSzfinal)=="val.16"]<- "v4"

write.csv(TumorSzfinal, "./cvresults/34-TumorSz.csv")
# ========================================================================
# PCA MODEL
# ========================================================================
TumorSzV3<-read.csv("./cvresults/34-TumorSz.csv")
keeps <-c("v2","v3")
...
a<- matrix(a)
colnames(a)<-paste0("model34")
write.csv(a,"./val_v3/34.csv", append = FALSE, quote = FALSE, sep = "", row.names = FALSE, col.names = TRUE)
scorevisit3<- cbind(USUBJID=TumorSzfinal$USUBJID, scorevisit3)


write.csv(scorevisit3,"./cvresults/34-pca3.csv")
# ========================================================================
keeps <- c("v2", "v3", "v4")
...
a<- matrix(a)
colnames(a)<-paste0("model34")
write.csv(a,"./val_v4/34.csv", append = FALSE, quote = FALSE, sep = "", row.names = FALSE, col.names = TRUE)
scorevisit4<- cbind(USUBJID=TumorSzfinal$USUBJID, scorevisit4)


write.csv(scorevisit4,"./cvresults/34-pca4.csv")
# =========================================================================
# Overall survival
# =========================================================================
...
write.csv(os, "./cvresults/34-os.csv")
# =================================================================
...
idx<- adrs$LSTEST=="Best Overall Response"
BestResp <- unique(data.frame(USUBJID=adrs$USUBJID[idx], BORval=adrs$LSSTRESC[idx]))
...
BestResp["BORval"][BestResp["BORval"]=="IR"]<-NA
...
write.csv(BestResp, "./cvresults/34-BestResp.csv")
# ==================================================================
# Merge all data  visits 
# ==================================================================
base<-read.csv("./cvresults/34-base.csv")
os<-read.csv("./cvresults/34-os.csv")
BestResp<-read.csv("./cvresults/34-BestResp.csv")
pca3<-read.csv("./cvresults/34-pca3.csv")
pca4<-read.csv("./cvresults/34-pca4.csv")
...
drop <- c("X.x","X.y")
write.csv(alldata, "./cvresults/34-alldata0.csv")
alldata<- merge(TumorSzfinal, alldata, by="USUBJID")
write.csv(alldata, "./cvresults/34-alldata34.csv")
alldata <- merge(os, alldata, by="USUBJID")
alldata = alldata[,!(names(alldata) %in% drop)]


write.csv(alldata, "./cvresults/34-alldata.csv")

# ============================================================================
# Prediction / Cross validation for PCA Model 3 Visits
# ============================================================================
alldata<-read.csv("./cvresults/34-alldata.csv")
...
glmnet.bin <- function(data,  idx.tr=NULL, base.covariates= baseCov, lambda=c("lambda.min", "lambda.1se")){
  ...
  # tidy(bestcut)
  # write.csv( tidy(bestcut),  "./Resultmethod1/bestcut_V3.csv")
  coef.exactt <- coef(cv.fit, s = c(lambda))
  
  # tidy(coef.exactt)
  # write.csv( tidy(coef.exactt),  "./cvresults/34-coef_V3.csv")
...
write.csv(conf_matrix, "./cvresults/34-PCA2_conf_matrix.csv")
...
write.table( bestcut,  "./cvresults/34bestcut_V3.csv")
write.table(coef_intercept, "./cvresults/34Intercept_V3.csv")
write.table(coef_sex, "./cvresults/34SEX_V3.csv")
write.table(coef_age, "./cvresults/34AGE_V3.csv")
write.table(coef_pc1, "./cvresults/34PC1_V3.csv")
write.csv(PCAV2Prediction, "./cvresults/34PCAV2Prediction_V3.csv")
write.table(misclassificationrate, "./cvresults/34misclassificationrate_V3.csv")
write.table(auc, "./cvresults/34auc_V3.csv")
write.table(sensitivity, "./cvresults/34sensitivity_V3.csv")
write.table(specificity, "./cvresults/34specificity_V3.csv")
# ============================================================================
...
glmnet.bin <- function(data, idx.tr=NULL, base.covariates= baseCov, lambda=c("lambda.min", "lambda.1se")){
  ...
  # tidy(bestcut)
  # write.csv( tidy(bestcut),  "./cvresults/34-bestcut_V4.csv")
  coef.exactt <- coef(cv.fit, s = c(lambda))
  
  # tidy(coef.exactt)
  # write.csv( tidy(coef.exactt),  "./cvresults/34-coef_V4.csv")
...
specificity<- specificity(conf_matrix)
conf_matrix<- as.numeric(conf_matrix)
write.csv(conf_matrix, "./cvresults/34-PCA3_conf_matrix.csv")
...
write.table( bestcut,  "./cvresults/34bestcut_V4.csv")
write.table(coef_intercept, "./cvresults/34Intercept_V4.csv")
write.table(coef_sex, "./cvresults/34SEX_V4.csv")
write.table(coef_age, "./cvresults/34AGE_V4.csv")
write.table(coef_pc1, "./cvresults/34PC1_V4.csv")
write.table(coef_pc2, "./cvresults/34PC2_V4.csv")
write.csv(PCAV3Prediction, "./cvresults/34PCAV3Prediction_V4.csv")
write.table(misclassificationrate, "./cvresults/34misclassificationrate_V4.csv")
write.table(auc, "./cvresults/34auc_V4.csv")
write.table(sensitivity, "./cvresults/34sensitivity_V4.csv")
write.table(specificity, "./cvresults/34specificity_V4.csv")
# ===================================================================================