# ===================================================================================================================== 
#  DESCRIPTION: 
#  The codes rate the each patient based on regression trees on each data set
# ===================================================================================================================== 
#  INPUTS:
#  - Segmentation (bad and good group)
#  - Score of each patients
#  - All data frames contains baseline characteristics, Principal component scores, BOR, OS
# ===================================================================================================================== 
#  OUTPUTS:
#  - Results of regression tree for each data set
# ===================================================================================================================== 
#  Codes produced by: 
#  - Elham Majd
# ===================================================================================================================== 
...
for(i in 1:100)
{
  ...
  do.call(file.remove,list(list.files("./csvlist2",full.names = TRUE)))
  ...
  for (h in names (s[1]))
  {
    for (j in names (k))
    {
      coefval[j]
      coefvisit[j]
      if (is.null(coefvisit[j][2,])) coefvisit[j][2,]<- 0
    ...
    fit <-
      rpart(
           dat$datasetfinal ~ dat[,1]+ dat[,2]+ dat[,3]+ dat[,4]+ dat[,5]+ dat[,6]+ dat[,7]+ dat[,8]+ dat[,9]+ dat[,10]+
          dat[,11]+ dat[,12]+ dat[,13]+ dat[,14]+ dat[,15]+ dat[,16]+ dat[,17],
        method = "anova",
        data = dat
      )
    ...
    pruned.tree <- prune(fit, cp = bestcp)
    prp(pruned.tree,
        faclen=0, 
        extra=1, 
        roundint=F, 
        digits=5) 
    pruned.tree$frame
    summary(residuals(pruned.tree))
    
    
    pred2<- pruned.tree$y
    
    pred<- predict(pruned.tree, dat[c(1:17)])
    pred
    ...
    write.table(pred, "./method3results/PredDT.csv", append = TRUE,
                quote = FALSE,
                 sep = ",",
                row.names = row.names,
                col.names = FALSE)
