...
# =========================================================================
Datasets= "./Datasets"  
filename0=paste0(Datasets,"/37/dm")
wd<- paste0(Datasets,"/37/dm.sas7bdat")
dm<- assign(filename0, read_sas(wd))


base <- unique(dm[,c("RPT" , "SEX", "AGE")])
names(base)[names(base) == "RPT"] <- "USUBJID"
...
write.csv(base,"./cvresults/37-base.csv")

# ==========================================================================
# Actual Tumor Size
# ==========================================================================
filename0=paste0(Datasets,"/37/tumor_m")
wd<- paste0(Datasets,"/37/tumor_m.sas7bdat")
dtr<- assign(filename0, read_sas(wd))


TumorSz <- data.frame( USUBJID=dtr$RPT, val=as.numeric(dtr$LENGTH) , visitN = dtr$VISIT)
...
PTumorch<- data.frame(USUBJID=TumorSz$USUBJID, val.1=TumorSz$val.1, TumorCh)
PTumorch[PTumorch == "Inf"] <- NA
PTumorch<- na.omit(PTumorch)

write.csv(PTumorch, "./cvresults/37-FullTumorSz.csv")


keeps <- c("USUBJID","val.6","val.9","val.12")

TumorSzfinal =PTumorch[keeps]
names(TumorSzfinal)[names(TumorSzfinal)=="val.6"] <-"v2"
names(TumorSzfinal)[names(TumorSzfinal)=="val.9"]<- "v3"
names(TumorSzfinal)[names(TumorSzfinal)=="val.12"]<- "v4"

write.csv(TumorSzfinal, "./cvresults/37-TumorSz.csv")
# ========================================================================
# PCA MODEL
# ========================================================================
TumorSzV3<-read.csv("./cvresults/37-TumorSz.csv")
keeps <-c("v2","v3")
...
a<- matrix(a)
colnames(a)<-paste0("model37")
write.csv(a,"./val_v3/37.csv", append = FALSE, quote = FALSE, sep = "", row.names = FALSE, col.names = TRUE)
scorevisit3<- cbind(USUBJID=TumorSzfinal$USUBJID, scorevisit3)


write.csv(scorevisit3,"./cvresults/37-pca3.csv")
# ========================================================================
keeps <- c("v2", "v3", "v4")
...
a<- matrix(a)
colnames(a)<-paste0("model37")
write.csv(a,"./val_v4/37.csv", append = FALSE, quote = FALSE, sep = "", row.names = FALSE, col.names = TRUE)
scorevisit4<- cbind(USUBJID=TumorSzfinal$USUBJID, scorevisit4)


write.csv(scorevisit4,"./cvresults/37-pca4.csv")
# =========================================================================
# Overall survival
# =========================================================================
...
write.csv(os, "./cvresults/37-os.csv")
# =================================================================
# best overall response T=CR, T=PR, F=SD, F=PD, NE -> NA
# ================================================================= 
filename0=paste0(Datasets,"/37/tumor_mresp")
wd<- paste0(Datasets,"/37/tumor_mresp.sas7bdat")
adrs<- assign(filename0, read_sas(wd))


BestResp <- (data.frame(USUBJID=adrs$RPT, BORval=adrs$BOVRN))


BestResp["BORval"][BestResp["BORval"]==1]<-1
BestResp["BORval"][BestResp["BORval"]==2]<-1
BestResp["BORval"][BestResp["BORval"]==3]<-0
BestResp["BORval"][BestResp["BORval"]==4]<-0
BestResp["BORval"][BestResp["BORval"]==19]<-NA
BestResp["BORval"][BestResp["BORval"]==95]<-NA
...
write.csv(BestRespf, "./cvresults/37-BestResp.csv")

# ==================================================================
# Merge all data  visits 
# ==================================================================
base<-read.csv("./cvresults/37-base.csv")
os<-read.csv("./cvresults/37-os.csv")
BestResp<-read.csv("./cvresults/37-BestResp.csv")
pca3<-read.csv("./cvresults/37-pca3.csv")
pca4<-read.csv("./cvresults/37-pca4.csv")
...
drop <- c("X.x","X.y")
write.csv(alldata, "./cvresults/37-alldata0.csv")
TumorSzfinal$USUBJID<- as.numeric(TumorSzfinal$USUBJID)
alldata<- merge(TumorSzfinal, alldata, by="USUBJID")
write.csv(alldata, "./cvresults/37-alldata37.csv")
os$USUBJID<- as.numeric(os$USUBJID)
# alldata <- merge(os, alldata, by="USUBJID")
alldata = alldata[,!(names(alldata) %in% drop)]


write.csv(alldata, "./cvresults/37-alldata.csv")
...
glmnet.bin <- function(data,  idx.tr=NULL, base.covariates= baseCov, lambda=c("lambda.min", "lambda.1se")){
  ...
  # tidy(bestcut)
  # write.csv( tidy(bestcut),  "./cvresults/37-bestcut_V3.csv")
  coef.exactt <- coef(cv.fit, s = c(lambda))
  
  # tidy(coef.exactt)
  # write.csv( tidy(coef.exactt),  "./cvresults/37-coef_V3.csv")
...
write.csv(conf_matrix, "./cvresults/37-PCA2_conf_matrix.csv")
...
write.table( bestcut,  "./cvresults/37bestcut_V3.csv")
write.table(coef_intercept, "./cvresults/37Intercept_V3.csv")
write.table(coef_sex, "./cvresults/37SEX_V3.csv")
write.table(coef_age, "./cvresults/37AGE_V3.csv")
write.table(coef_pc1, "./cvresults/37PC1_V3.csv")
write.csv(PCAV2Prediction, "./cvresults/37PCAV2Prediction_V3.csv")
write.table(misclassificationrate, "./cvresults/37misclassificationrate_V3.csv")
write.table(auc, "./cvresults/37auc_V3.csv")
write.table(sensitivity, "./cvresults/37sensitivity_V3.csv")
write.table(specificity, "./cvresults/37specificity_V3.csv")
# ============================================================================
...
glmnet.bin <- function(data, idx.tr=NULL, base.covariates= baseCov, lambda=c("lambda.min", "lambda.1se")){
  ...
  # tidy(bestcut)
  # write.csv( tidy(bestcut),  "./cvresults/37-bestcut_V4.csv")
  coef.exactt <- coef(cv.fit, s = c(lambda))
  
  # tidy(coef.exactt)
  # write.csv( tidy(coef.exactt),  "./cvresults/37-coef_V4.csv")
...
specificity<- specificity(conf_matrix)
conf_matrix<- as.numeric(conf_matrix)
write.csv(conf_matrix, "./cvresults/37-PCA3_conf_matrix.csv")
...
write.table( bestcut,  "./cvresults/37bestcut_V4.csv")
write.table(coef_intercept, "./cvresults/37Intercept_V4.csv")
write.table(coef_sex, "./cvresults/37SEX_V4.csv")
write.table(coef_age, "./cvresults/37AGE_V4.csv")
write.table(coef_pc1, "./cvresults/37PC1_V4.csv")
write.table(coef_pc2, "./cvresults/37PC2_V4.csv")
write.csv(PCAV3Prediction, "./cvresults/37PCAV3Prediction_V4.csv")
write.table(misclassificationrate, "./cvresults/37misclassificationrate_V4.csv")
write.table(auc, "./cvresults/37auc_V4.csv")
write.table(sensitivity, "./cvresults/37sensitivity_V4.csv")
write.table(specificity, "./cvresults/37specificity_V4.csv")
# ================================================================================